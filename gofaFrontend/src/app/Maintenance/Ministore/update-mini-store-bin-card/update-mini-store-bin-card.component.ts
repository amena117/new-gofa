...
export class UpdateMiniStoreBinCardComponent implements OnInit {
  ...
  updateSerialNumber(index: number, value: string) {
    this.serialNumbers[index] = value.trim();
  }
  ...
  onReceive() {
    if (
      !this.miniStoreBinCard.stockNumber ||
      !this.miniStoreBinCard.model ||
      !this.quantity ||
      !this.miniStoreBinCard.recievedFrom
    ) {
