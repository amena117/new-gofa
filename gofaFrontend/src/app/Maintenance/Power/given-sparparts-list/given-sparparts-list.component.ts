...
export class GivenSparpartsListComponent implements OnInit {
  ...
  applyRoleBasedFilter(): void {
    ...
    switch (this.userRole) {
      ...
      case 'RTEAM_LEADER':
        filteredRequests = this.allSparePartsRequests.filter(req =>
          maintenanceTypeMap.get(req.worksOrderNumber) === 'RADIO_MAINTENANCE' &&
          req.approvedBy === 'MINISTORE' &&
          req.requestType === 'RADIO_MAINTENANCE'
        );
        break;
      case 'HTEAM_LEADER':
        filteredRequests = this.allSparePartsRequests.filter(req =>
  ...
  getActionButtonLabel(): string {
    if (['POWER', 'OFFICE_MACHINE', 'RADIO_MAINTENANCE'].includes(this.userRole)) return 'Maintain';
    if (['PTEAM_LEADER', 'OTEAM_LEADER', 'RTEAM_LEADER'].includes(this.userRole)) return 'Approve';
    return 'Action';
  }
  ...
  openForm(request: SparePartsRequest): void {
    ...
    this.isMaintenanceMode = ['POWER', 'OFFICE_MACHINE', 'RADIO_MAINTENANCE'].includes(this.userRole);
